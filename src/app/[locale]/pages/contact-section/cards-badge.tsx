...
export const CardsBadge = ({ icon, title, subtitle, action, link }: CardsBadgeProps) => {
    return (
        <div className="bg-gray-200/70 dark:bg-[#141414] p-6 rounded-lg w-[290px]">
            <div className="text-5xl text-purple-400 flex justify-center">
                {icon}
            </div>
            <div className="">
                <h1>{title}</h1>
                ...
                <a href={link} className="flex justify-center items-center space-x-1 group">
                    <h4 className="transition-all group-hover:text-purple-400">{action}</h4>
                    <IoIosArrowForward className="transition-all group-hover:text-purple-400" />
                </a>
            </div>
